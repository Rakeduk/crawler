...
#by Radek Dudek and Wiktor Jankowski
...
class LIFO_Policy:
    def __init__(self):
        self.queue = ['http://www.cs.put.poznan.pl/mtomczyk/ir/lab1/exercise2/s0.html']
    
    def getURL(self, c, iteration):
        if self.queue:
            return self.queue.pop()
        else:
            for item in c.seedURLs:
                self.queue.append(item)      
            
    def updateURLs(self, c, retrievedURLs, retrievedURLsWD, iteration):
        retrievedURLsList = list(retrievedURLs)
        retrievedURLsList.sort(key=lambda url: url[len(url) - url[::-1].index('/'):])
        for item in retrievedURLsList:
            self.queue.append(item)
...
def getNormalisedURLs(retrievedURLs):
    lowerCaseRetrievedURLs = [item.lower() for item in retrievedURLs] 
    return set(lowerCaseRetrievedURLs)
